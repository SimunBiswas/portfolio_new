...
export const IntroBg_1: React.FC<IntroBgProps> = ({ onFinish }) => {
  const headings = [
    "AVINASH MASIH",
    "DESTRUCTION ARTIST",
    "GAME DEVELOPER",
  ...
  // ⏱ Trigger onFinish after animation duration (~9 + 2.5)
  useEffect(() => {
    const timer = setTimeout(() => {
      onFinish?.();
    }, 9000); // 9s delay + 2.5s scale-out
  ...
  return (
      ...
      // ${visibility ? 'block' : 'hidden'}
        ...
        <motion.div  className="relative mt-[8vh] md:mt-[10vh] text-[clamp(0.9rem,1.5vw,1.5rem)] text-[#FFFFFF]/35 text-center z-20"
        ...
        {/* ⚡ Progress Bar */}
        <motion.div className="relative mt-[2vh] w-[90%] max-w-full z-20"
        animate={{ opacity: [1, 1, 0] }}
        transition={{ duration: 1.5, ease: "easeOut", delay: 8, times: [0, 0.8, 1] }}
        >
        ...
        {/* 🧠 WORKSPACE Animation */}
        <motion.div  className="relative w-full flex flex-col justify-center items-center text-center mt-[5vh] z-30"
        animate={{ opacity: [1, 1, 0] }}
        transition={{ duration: 1.5, ease: "easeOut", delay: 8, times: [0, 0.8, 1] }}
        >
          {works.map((work, i) => (
